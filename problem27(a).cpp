...
{
    int n=0;
    cout<<"How much is the population size for this combinations: ";
    cin >> n;
    vector<char>population={};
    for(int count=0;count<n;count++){
        char temp;
        cout<<"Write "<<count+1<<" element: ";
        cin >> temp;
        population.push_back(temp);
    }
    cout<<"How many values of r you want in nCr: ";
    cin >> n;
    for(int count=0;count<n;count++){
        cout<<"Enter value "<<count+1<<": ";
        int temp;
        cin >> temp;
        cout<<"Combinations for value of r="<<temp<<" are:-"<<endl;
        combinations(temp,population);
    }
}